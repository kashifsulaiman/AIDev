...
export function useFetchGithubRepos(token: string, onSuccess?: (data: Repo[]) => void, onError?: (error: any) => void) {
    return useMutation<Repo[], unknown, void>(
        async () => {
            ...
            return response.data.map((repo: { full_name: string }) => ({ label: repo.full_name }));
        },
        {
            onSuccess,
            onError,
...
export async function fetchAllFiles(reponame: string, token: string) {
    ...
    try {
        ...
        const branchResponse = await axios.get(`${apiBaseUrl}/branches/${defaultBranch}`, { headers });
        ...
        const allItems = treeResponse.data.tree.map((item: { path: string; type: string, size: number }) => ({
            name: item.path,
...
export async function fetchSelectedFiles(
    ...
    async function fetchRepoTree(branch: string) {
        const response = await axios.get(`${apiBaseUrl}/git/trees/${branch}?recursive=1`, { headers });
        return response.data.tree;
    }

    async function fetchFileContent(fileSha: string) {
        const response = await axios.get(`${apiBaseUrl}/git/blobs/${fileSha}`, { headers });
