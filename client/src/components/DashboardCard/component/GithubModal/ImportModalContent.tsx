...
export default function ImportModalContent({ repos, modalCloseHandler }: ImportModalContentInterface) {

    const router = useRouter();
    const [loading, setLoading] = useState<boolean>(false)
    const [selectedRepo, setSelectedRepo] = useState<SelectedRepoType>({ label: 'Select repository to import' });
    const [repoItems, setRepoItems] = useState<RepoItemsType[] | null>(null)
    const [selectedItems, setSelectedItems] = useState<RepoItemsType[] | null>(null);
    ...
    const getRepoData = async () => {
        ...
        setLoading(true)
        try {
            const filesOnly = selectedItems && selectedItems.filter(item => item.type !== "folder");
            const data = await fetchSelectedFiles(selectedRepo.label, githubAuth.token, filesOnly)
    ...
    useEffect(() => { getRepoData() }, [selectedItems])
    ...
    return (
        <>
            ...
            {loading
                ...
                : repoItems && <FileManagerSection selectedRepo={selectedRepo} repoItems={repoItems} selectedItems={selectedItems} setSelectedItems={setSelectedItems} />}
